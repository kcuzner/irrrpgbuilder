...
//Tries to convert  accented characters 
irr::core::stringw convert(irr::core::stringc text)
{
	...
	{
		// Characters that goes over 128 (extended characters) get < 0 dont' know why but fix it.
		if ((int)text[a]<0)
		{
			int ts = 256+(int)text[a];
			text[a]=char(256+ts);
			
			line.append(text[a]);

			//printf("Under 0 code?: %d, %d\n",(int)text[a],ts);
		} else
